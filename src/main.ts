  ...
  <div class="shell">
    ...
    <main id="main" class="main-grid">
      <section class="hero card">
        ...
        <p class="lead">Xteink には、行頭が2文字分下がる、ルビ付きの行が表示されない、罫線が横向きのままになるといった仕様があります。本Webアプリでは、これらの問題を解消するため、以下の内容でEPUBを変換します。</p>
        <ul class="feature-list" role="list">
          <li>ルビタグ を「漢字（かんじ）」のように変換します</li>
          <li>各 p タグ先頭に空 span を追加します。これによりXteink側に p タグを誤認させ、行頭の字下げを文章側にて入られた全角空白を見るようにします</li>
          <li>縦書き表示で横向きになりやすい罫線・括弧・記号を縦書き用Unicode文字に置換します（narou.rb の変換ロジックに準拠）</li>
      ...
      <section class="panel card uploader-panel">
        ...
        <div class="toggles">
          ...
          <label class="toggle"><input id="verticalCharsToggle" type="checkbox" />縦書き向け文字変換（罫線・括弧・記号を縦組み用へ）</label>
        </div>
        <div class="actions">
      ...
      <section class="panel card stats-panel">
        ...
        <div class="stats-grid">
          ...
          <article class="stat"><span class="stat-label">縦字変換</span><strong id="statTate">0</strong></article>
...
const verticalCharsToggle = document.querySelector<HTMLInputElement>('#verticalCharsToggle')!;
...
const statTate = document.querySelector<HTMLElement>('#statTate')!;
  ...
  worker.postMessage({
    ...
    payload: {
      ...
      options: {
        ...
        convertVerticalChars: verticalCharsToggle.checked
      }
    }
  }, [buffer]);
});
...
worker.addEventListener('message', (event: MessageEvent) => {
  const data = event.data as
    ...
    | { type: 'done'; payload: { blob: Blob; fileName: string; summary: { htmlFiles: number; rubyConversions: number; spanInsertions: number; verticalCharConversions: number; warnings: string[]; logs: string[] } } }
  ...
  if (data.type === 'done') {
    ...
    statTate.textContent = String(data.payload.summary.verticalCharConversions);
